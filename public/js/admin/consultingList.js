    ...
    if(!IsBull){
        for(let i = 0; i < max_list_length; i++){
            if(i >= data.length){
                ...
                <td>${data[i].No}</td>
                <td><a href="/consultingList/${data[i].No}">${data[i].category}</a></td>
                <td><a href="/consultingList/${data[i].No}">${data[i].name}</a></td>
                <td><a href="/consultingList/${data[i].No}">${data[i].cellphone}</a></td>
                <td><a href="/consultingList/${data[i].No}">${data[i].use}</a></td>
                <td><a href="/consultingList/${data[i].No}">${data[i].consultant}</a></td>
                <td><a href="/consultingList/${data[i].No}">${data[i].assignment}</a></td>
                <td><a href="/consultingList/${data[i].No}">${data[i].contract}</a></td>
                <td><a href="/consultingList/${data[i].No}">${data[i].visit}</a></td>
                <td><a href="/consultingList/${data[i].No}">${data[i].insert_date}</a></td>
                </tr>
                `
            }
        }
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            if(data.length <= 0){
                template = `
                    <tr>
                        <td colspan="${table_colspan}">상담 없음</td>
                    </tr>
                `
            }else{
...
$(function(){
    ...
    join_btn.on("click", function(){
        ...
        const consultant_url = "/consultant_list";
        const consultant_dataType = "json";
        $.ajax({
            url:consultant_url,
            dataType:consultant_dataType,            
            success:function(data){
                let template = ""
                for(let i = 0; i < data.length; i++){
                    template += `<option value="${data[i].name}">${data[i].name}</option>`;
                }
                consultant.append(template);
            }
        });
    });
